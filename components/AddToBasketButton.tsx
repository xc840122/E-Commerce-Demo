...
function AddToBasketButton({ product, isDisabled }: AddToBasketButtonProps) {
  const { addItem, removeItem, getItemCount } = useBasketStore();
  const itemCount = getItemCount(product._id);

  const [isClient, setIsClient] = useState(false);

  // Use useEffect to set isClient to true after compontent mounts
  ...
  return (
    <div className="flex items-center justify-center space-x-2">
      <button
        ...
        disabled={itemCount <= 0 || isDisabled}
      ...
      <button
        ...
        disabled={isDisabled}
        className={`w-8 h-8 rounded-full flex items-center justify-center
          transition-colors duration-200 ${isDisabled
